...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      const reserved = await TicketStockModel.reserveTickets(ticketType, quantity, expiresAt, orderId)
      if (!reserved) {
        return NextResponse.json({ error: "Failed to reserve tickets" }, { status: 400 })
      }

      console.log(`✅ Reserved ${quantity}x ${ticketType} tickets for order ${orderId}`)
      ...
      console.log(`✅ Created payment for order ${orderId}: ${payment.payment_id}`)
      ...
      console.error("Error in payment creation, releasing reservation:", error)
      await TicketStockModel.releaseReservation(ticketType, quantity, orderId)
      throw error
    }
  } catch (error) {
